...
muertes_df = (
    pd.read_csv(dir_series + "covid19_mex_muertes_nuevas.csv")
...
casos_df = (
    pd.read_csv(dir_series + "covid19_mex_casos_nuevos.csv")
    .set_index("Fecha")["Nacional"]
    .rename("Nuevos_JH")
)
...
muertes_abiertos_df = (
    pd.read_csv(dir_series_abiertos + "covid19_mex_muertes.csv")
    .set_index("Fecha")["Nacional"]
...
casos_abiertos_df = (
    pd.read_csv(dir_series_abiertos + "covid19_mex_confirmados.csv")
    .set_index("Fecha")["Nacional"]
...
# Creamos dfs
muertes_nuevas = (
    pd.concat((muertes_df, muertes_abiertos_df), axis=1).fillna(0).astype(int)
)
muertes_acumuladas = muertes_nuevas.cumsum()

# agregamos el promedio
muertes_promedio = (
    muertes_nuevas.rolling(window=rolling_window, center=False).mean().round(2)
)  # 2 decimales
...
# el promedio
casos_promedio = (
    casos_nuevos.rolling(window=rolling_window, center=False).mean().round(2)
)  # 2 decimales
